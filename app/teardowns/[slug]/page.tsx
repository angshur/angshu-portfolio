...
export async function generateStaticParams() {
  return getAll('teardowns').map((p) => ({ slug: p.slug }))
}

export default function TeardownPost({ params }: { params: { slug: string } }) {
  let item
  try {
    item = getBySlug('teardowns', params.slug)
  } catch {
    notFound()
  }
    ...
    <Section>
      <Container>
        <div className="max-w-3xl">
          <p className="text-sm text-slate-500 dark:text-slate-400">Teardowns</p>
